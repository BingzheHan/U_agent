/*
 * user agent common helper functions, etc.
...
{
	static char ssid_txt[50 * 4 + 1]; //For GBK ssid the max size is 16*3, add additional 2
...
#ifdef CONFIG_GB2312
	u8 ssid_new[MAX_SSID_UTF8_BUF_LEN+4];
	int ssid_new_len = 0;
	os_memset(ssid_new, 0, MAX_SSID_UTF8_BUF_LEN+4);
	os_memcpy(ssid_new, ssid, ssid_len);
	ssid_new_len = ssid_len;
	check_gb2312_change_utf8((char*)ssid_new, &ssid_new_len);
	printf_encode(ssid_txt, sizeof(ssid_txt), ssid_new, ssid_new_len);
#else
